...
function draw() {
  ...
  let margin = rnd.random_choice([0, 64, 128, 256]) * m;
  ...
  if (duo) {
    ...
    while (palette.colors.length > 2) {
      const index = floor(rnd.random_dec() * palette.colors.length);
  ...
  initAngleGrid(leftToRight, wobbly, straight);
  ...
  loop1: for (let i = 0; i < polys.length; i++) {
    ...
    if (group && !duo) {
      if (i % mod === 0) {
        ...
        color = color(palette.colors[Math.ceil((i / 12) % mod) - 1]);
      } else {
        color = color(
      ...
      color = color(palette.colors[i % mod]);
    } else {
      color = color(
        palette.colors[rnd.random_int(0, palette.colors.length - 1)]
      );
...
function initAngleGrid(leftToRight, wobbly, straight) {
  cols = floor(width / w);
  rows = floor(height / w);
  ...
  for (let x = 0; x < cols + 1; x++) {
    ...
    for (let y = 0; y < rows + 1; y++) {
      ...
      const angle = map(
        ...
        0.0,
        1.0,
        leftToRight ? (4 / 3) * PI : (2 / 3) * PI,
        leftToRight ? (2 / 3) * PI : (4 / 3) * PI
      );
...
function drawBorder(e, color, stroke) {
  fill(color);
  stroke(stroke);
  strokeWeight(2 * m);
  strokeJoin(MITER);
