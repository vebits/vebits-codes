...
function Project() {
  ...
  const draw = (p) => {
    let remainingWidth = dim;
    let xoff = 0;

    /* while (remainingWidth > 0) {
      const stripeWidth = r.random_int(5, 10);
      //p.noStroke();
      p.fill(palette.colors[r.random_int(0, palette.size - 1)]);
      p.rect(xoff, 0, stripeWidth, dim);

      xoff += stripeWidth;
      remainingWidth -= stripeWidth;
    } */

    /*     let remainingHeight = 500;
    let yoff = 0;
    while (remainingHeight > 0) {
      const stripeWidth = r.random_int(10, 20);
      p.fill(palette.colors[r.random_int(0, palette.size - 1)]);

      p.rect(0, yoff, dim, stripeWidth);

      yoff += stripeWidth;
      remainingHeight -= stripeWidth;
    } */
