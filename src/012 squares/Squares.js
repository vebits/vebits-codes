...
function Squares() {
  const setup = (p, canvasParentRef) => {
    ...
    p.background(
      p.color(hex2hsl(palette[rnd.random_int(0, palette.length - 1)])[0])
    );
  ...
  const draw = (p) => {
    p.stroke(100);
    for (let i = 0; i < 3; i++) {
      drawRect(p, rnd.random_between(0, width), rnd.random_between());
    }
  };

  function drawRect(p, x1, y1, x2, y2) {
    for (let i = 0; i < 500; i++) {
      p.line(
        rnd.random_between(x1, x2),
        y1 + rnd.random_between(0, 10),
        rnd.random_between(x1, x2),
        y2 + rnd.random_between(0, 10)
      );
    }
    for (let i = 0; i < 500; i++) {
      p.line(
        x1 + rnd.random_between(0, 10),
        rnd.random_between(y1, y2),
        x2 + rnd.random_between(0, 10),
        rnd.random_between(y1, y2)
      );
    }
  }
